...
class EventsViewsets(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_judge:
            return Events.objects.filter(attendees__in=["judges","both"],schedule_date__gte = date).annotate(
               day_mod=Cast('schedule_date', DateField())).order_by('day_mod',"-id")
        return Events.objects.filter(schedule_date__gte = date).annotate(day_mod = Cast('schedule_date', DateField())).order_by('day_mod',"-id")
    ...
    def create(self,request,*args,**kwargs):
        ...
        for user in users:
            ...
            dt={
                        "title":"New event is created by admin",
                        "description":"New event is created by admin",
                        "created_for": user,
                        "type":"response",
