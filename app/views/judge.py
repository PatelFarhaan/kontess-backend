...
class JudgeRequestViewsets(viewsets.ModelViewSet):
    ...
    def _update(self,request,*args,**kwargs):
        ...
        try:
            ...
            email_message = EmailMultiAlternatives("Kontess Judge Account email",'',settings.EMAIL_HOST_USER,[obj.judge.user.email],from_email=obj.created_for.email)
        ...
        return Response({"msg":"Judge request updated sucessfuly.","status":status.HTTP_200_OK},status=status.HTTP_200_OK)
...
class JudgeRequestTeamViewsets(viewsets.ModelViewSet):   
    ...
    def jrt_update(self,request,*args,**kwargs):
        ...
        obj.save()
        dt = {
            "title":"Your request for join team {} as judge has been {} by admin".format(obj.team.name,request.data.get("status")),
            "description":"Your request for join team {} as judge has been {} by admin".format(obj.team.name,request.data.get("status")),
