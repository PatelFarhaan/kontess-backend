...
class AnnouncementViewsets(viewsets.ModelViewSet):
    ...
    def _update(self,request,pk=None):
        ...
        obj.save()
        return Response({"msg":"Announcement update successfully.","status":status.HTTP_200_OK},status=status.HTTP_200_OK)
...
class AnnouncementStatusViewsets(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return AnnouncementStatus.objects.filter(**_star,user=self.request.user).order_by("-id")
