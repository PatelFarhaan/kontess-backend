...
def email_sending(data,user):
    ...
    email_message = EmailMultiAlternatives(data.get("title"),data.get("description"),settings.EMAIL_HOST_USER,[user.email])
...
def create_notification(task):
    dt={
        "title":"Your submisstion is due on this task {}".format(task.task.title),
        "description":"Your submisstion is due on this task {} plese submit as soon as possible".format(task.task.title),
        "type":"response",
        "req_data":{}
    }
    Notification.objects.create(created_by=get_admin_user(),created_for= task.participant.user,**dt)
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        if get_participants_task():
            ...
            print("Email and notification genrated successfuly")    
        else:
            print("No task found.") 