...
def get_participants_task():
    ...
    tasks = ParticipantTask.objects.annotate(day_mod=Cast('task__grade_due_date', DateField())).filter(status__in["submitted","resubmit"],day_mod=date)
    return tasks
...
def create_notification(task):
    dt={
        "title":"Grading is due today on this task {}".format(task.task.title),
        "description":"Grading is due today on this task {}".format(task.task.title),
    }
