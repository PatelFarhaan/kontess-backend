...
class EventsViewsets(viewsets.ModelViewSet):
    ...
    @action(detail=False,methods=["get"],url_path="zoom-events")
    def zoom_events(self,request,*args,**kwargs):
        data = Events.objects.exclude(meeting_id__isnull=True)#.exclude(meeting_id__isnull="") # TODO Change to false
        return Response({"data":self.get_serializer(data, many=True).data,"status":status.HTTP_200_OK},status=status.HTTP_200_OK)
