...
class ParticipantViewSet(viewsets.ModelViewSet):
    ...
    def create_team_request(self, request):
        ...
        if tr:
            ...
            if tr.team.created_by.team_join_request:
                try:
                    html_message="<html><body><strong><a href='/dashboard/profile/{0}'>{1}</a></strong> sent you a request to join team <strong><a href='/dashboard/team_view/{2}'>{3}</a></strong><div></body></html>".format(p.user.id,p.user.full_name,tr.team.id,tr.team.name)
                    email_message = EmailMultiAlternatives("Joining request for team".format(request.data.get("status")),'',settings.EMAIL_HOST_EMAIL,[tr.team.created_by.email])
